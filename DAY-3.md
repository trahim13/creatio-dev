...
# DB View + Object schema: *Represents structure of database view* (Represent in storage)

Короткая заметка про фичу Creatio, когда объект в конфигурации **не создаёт таблицу**, а **“представляет” (represents) уже существующее представление (VIEW) в БД**.

> В UI это обычно выглядит как чекбокс вида **“Represents structure of database view”**.

---

## Когда использовать

Используй **View-object** (объект, который представляет DB VIEW), когда нужен **read-model / витрина**:

- быстрые списки/разделы, где данные собираются из нескольких таблиц (JOIN);
- агрегаты (COUNT/SUM/MAX), вычисляемые поля, “денормализация” для UI;
- отчёты и выгрузки, где ESQ/ORM запрос получался бы слишком тяжёлым;
- “скрыть” сложную SQL-логику от UI: UI читает один объект, а внутри это VIEW.

**Не используйте** это как основной путь для CRUD, если ожидаются вставки/апдейты/бизнес-логика по объекту.

---

## Что делает флаг *Represents … view / Represent in storage*

Если флаг включён:

- Creatio **не генерирует** таблицу для объекта (DDL на стороне Creatio не выполняется).
- Объект в Creatio становится **проекцией** на существующее `VIEW` в PostgreSQL.
- Все выборки идут как `SELECT ... FROM <view>`.

---

## Минимальные требования

1) **Колонка `Id` обязательна**  
   VIEW должен возвращать UUID `Id` (лучше — `Id` базовой таблицы).

2) **`Id` должен быть стабильным**  
   Нельзя делать `Id = gen_random_uuid()` — строки будут “прыгать” и UI/ORM будет ломаться.

3) **Типы и имена колонок должны совпадать**  
   Колонки, описанные в Object schema, должны реально возвращаться VIEW (имя/тип/nullable).
---

## Как завести в Creatio (коротко)

1) Создай VIEW в PostgreSQL (редко меняется → обычно в SQL-скрипте/миграции).
2) В Creatio создай **Object schema** с именем, соответствующим VIEW.
3) Поставь флаг **Represents structure of database view**.
4) Добавь колонки:
  - `Id` (UUID, system)
  - `etc..` 
5) Используй объект в UI как **read-only источник** для списков/страниц/выборок.

---

## Практические заметки

- View-object отлично подходит как “витрина” для UI, но **лучше не строить на нём процессы записи**.
- Если всё-таки нужен CRUD — делай обычный объект (таблица) и отдельный VIEW только для чтения/аналитики.
- При изменении структуры VIEW нужно синхронизировать Object schema (и наоборот).
